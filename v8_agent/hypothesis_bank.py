...
class HypothesisBank:
        ...
        *,
        ...
        for index, raw in enumerate(hypotheses):
            ...
            hypothesis_id = stable_hash((
                ...
                raw.get("id"),
                claim,
                [(step.action_id, step.target_object_ids, step.coordinate_candidate_id, step.contract_kind) for step in plan],
            ), "hyp_")
    ...
    def update(self, judgment: Judgment) -> None:
        ...
        mechanics_mismatch = judgment.reason_code in {
        ...
        if mechanics_mismatch and "qwen" in item.source:
            ...
            if self._active_hypothesis_id == item.hypothesis_id:
                self._active_hypothesis_id = None
        if judgment.truth is TriTruth.TRUE and judgment.progress is Progress.POSITIVE:
            item.priority += 2.0
        ...
        elif judgment.truth is TriTruth.FALSE or judgment.validity is Validity.INVALID:
            item.validity = Validity.INVALID
            if item not in self.rejected:
                self.rejected.append(item)
...
def _v87_has_bound_coordinate_action(raw: dict[str, Any], packet: dict[str, Any] | None) -> bool:
    ...
    coordinate_action_ids = {
        str(item.get("id"))
        for item in ((packet.get("action_surface") or {}).get("actions") or [])
