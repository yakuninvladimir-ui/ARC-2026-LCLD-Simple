...
class QwenPacketBuilder:
    ...
    def _build_packet(self, snapshot: ARGALiteSnapshot, memory: "GameMemory", bank: "HypothesisBank", role: QwenRole, config: V8Config) -> dict[str, Any]:
        ...
        for item in objects:
            item.pop("local_hex_rows", None)
            item.pop("local_mask_omitted", None)
        relations = [_factual_relation(item) for item in _alias_relations(focus_relations, aliases)]
        candidates = [_factual_coordinate_candidate(item) for item in _alias_candidates(focus_candidates, aliases)]
        geometry_groups = _alias_geometry_groups(focus_geometry_groups, aliases)
        component_graph = _layered_component_graph_for_packet(snapshot, focus_ids, aliases, config)
        hex_patches = _hex_patches(
            snapshot,
            focus_objects,
            nested_component_view_ids,
            aliases,
            component_graph,
            config,
        )
        ...
        packet = {
            ...
            "action_diffs": _action_diffs(
                ...
                aliases,
                config,
                object_rebindings=object_rebindings,
            ),
            "memory": {
                "confirmed_effects": _confirmed_effect_memory(memory, aliases, config),
                "completed_levels": _completed_level_memory(memory, snapshot, config),
...
def _layered_focus_object_ids(
    ...
    limit = max(1, int(config.max_semantic_objects_in_packet))
    valid = {str(item.get("id")) for item in objects}
    ordered: list[str] = []

    def add(value: Any) -> None:
        object_id = str(value or "")
        if object_id in valid and object_id not in ordered and len(ordered) < limit:
            ordered.append(object_id)

    for binding in reversed(list(memory.semantic_bindings.values())):
        if binding.level_index != snapshot.level_index:
            continue
        resolved = memory.resolve_semantic_binding_references(binding, snapshot)
        for object_id in (*resolved["source_object_ids"], *resolved["reference_object_ids"]):
            add(object_id)
    for evaluation in reversed(memory.trajectory_evaluations):
        if evaluation.level_index != snapshot.level_index:
            continue
        binding = memory.semantic_bindings.get(str(evaluation.binding_id or ""))
        resolved = memory.resolve_semantic_binding_references(binding, snapshot) if binding is not None else None
        object_ids = (
            (*resolved["source_object_ids"], *resolved["reference_object_ids"])
            if resolved is not None
            else (*evaluation.source_object_ids, *evaluation.reference_object_ids)
        )
        for object_id in object_ids:
            add(object_id)
    for record in reversed(memory.action_memory_records):
        if not _record_matches_snapshot_level(record, snapshot):
            continue
        for change in record.get("object_deltas") or []:
            if isinstance(change, dict):
                add(change.get("object_id"))
    for candidate in candidates:
        add(candidate.get("object_id"))
    for item in objects:
...
def _factual_relation(item: dict[str, Any]) -> dict[str, Any]:
    return {
        key: value
        for key, value in item.items()
        if key not in {"satisfied", "parser_confidence", "salience_score"}
    }
...
def _layered_component_graph_for_packet(
    snapshot: ARGALiteSnapshot,
    focus_ids: set[str],
    ...
    raw = build_component_graph(
        ...
        max_boundary_corners=0,
    )
    component_id_map = {
        str(component.get("id")): f"cmp{index}"
    ...
    for component in raw.get("components") or []:
        ...
        projected = {
            "id": component_id_map.get(str(component.get("id")), str(component.get("id"))),
            "color": component.get("color"),
            "area": component.get("area"),
            "bbox_xyxy": component.get("bbox_xyxy"),
            "centroid_xy": component.get("centroid_xy"),
            "fill_ratio": component.get("fill_ratio"),
            "shape_hash": component.get("shape_hash"),
            "hole_count": component.get("hole_count"),
            "edge_contacts": component.get("edge_contacts") or [],
            "object_refs": component.get("object_refs") or [],
            "parent": component_id_map.get(str(parent)) if parent is not None else None,
            "normalized_shape_runs": component.get("normalized_shape_runs"),
        }
        compact = {key: value for key, value in projected.items() if value not in (None, [], {})}
        # Keep this field explicit: an empty list means the component belongs to
        # the global frame decomposition but is not a retained planning object.
        compact["object_refs"] = projected["object_refs"]
        components.append(compact)
    adjacency = []
    for edge in raw.get("adjacency") or []:
        if not isinstance(edge, dict):
            continue
        adjacency.append({
            "a": component_id_map.get(str(edge.get("a")), str(edge.get("a"))),
            "b": component_id_map.get(str(edge.get("b")), str(edge.get("b"))),
            "shared_edge_cells": int(edge.get("shared_edge_cells") or 0),
        })
    same_shape_groups = []
    for group in raw.get("same_shape_groups") or []:
        if not isinstance(group, dict):
            continue
        same_shape_groups.append({
            "shape_hash": group.get("shape_hash"),
            "component_ids": [component_id_map.get(str(value), str(value)) for value in group.get("component_ids") or []],
    ...
    return {
        ...
        "component_count_total": int(raw.get("component_count_total") or 0),
        "omitted_component_count": int(raw.get("omitted_component_count") or 0),
...
def _hex_patches(
    ...
    for component in (component_graph or {}).get("components") or []:
        if not isinstance(component, dict):
            continue
        for object_id in component.get("object_refs") or []:
            component_refs.setdefault(str(object_id), []).append(component)

    snapshot_by_id = {str(obj.object_id): obj for obj in snapshot.objects}
    suppressed_cells = {
        object_id: _record_occupied_cells(snapshot_by_id[object_id])
        for object_id in suppressed_object_ids
        if object_id in snapshot_by_id
    }
    occupied = {
        str(item.get("id")): _packet_object_occupied_cells(item)
        for item in objects
    }
    patches: list[dict[str, Any]] = []
    for item in objects:
        internal_id = str(item.get("id"))
        alias = object_aliases.get(internal_id)
        if alias is None:
            continue
        reasons: list[str] = []
        if len(item.get("palette_histogram") or {}) > 1:
            reasons.append("MULTICOLOR_OBJECT")
        linked_components = component_refs.get(alias, [])
        if len(linked_components) > 1:
            reasons.append("MULTIPLE_COMPONENTS_REFERENCE_OBJECT")
        if any(len(component.get("object_refs") or []) > 1 for component in linked_components):
            reasons.append("COMPONENT_REFERENCES_MULTIPLE_OBJECTS")
        own_cells = occupied.get(internal_id, frozenset())
        if any(
            own_cells.intersection(other_cells)
            for other_id, other_cells in occupied.items()
            if other_id != internal_id
        ):
            reasons.append("OVERLAPPING_CANONICAL_OBJECT_MASKS")
        if any(child_cells and child_cells < own_cells for child_cells in suppressed_cells.values()):
            reasons.append("SUPPRESSED_NESTED_COMPONENT_VIEW")
        if not reasons:
            continue
        object_bbox = list(item.get("bbox_xyxy") or [])
        if len(object_bbox) != 4:
            continue
        patch_bbox, clipped = _bounded_patch_bbox(
            object_bbox,
            snapshot.width,
            snapshot.height,
            max_side=config.max_hex_patch_side,
            padding=1,
        )
        ...
        patches.append({
            ...
            "patch_bbox_xyxy": patch_bbox,
            "clipped": clipped,
            "encoding": "categorical_hex_rows_0-F",
            "hex_rows": [row[x0 : x1 + 1] for row in snapshot.full_grid_hex_rows[y0 : y1 + 1]],
        })
        if len(patches) >= max(0, int(config.max_hex_patches_in_packet)):
            break
    return patches


def _packet_object_occupied_cells(item: dict[str, Any]) -> frozenset[tuple[int, int]]:
    bbox = item.get("bbox_xyxy") or []
    rows = item.get("local_hex_rows") or []
...
def _bounded_patch_bbox(
    ...
    *,
    ...
    ox0, oy0, ox1, oy1 = [int(value) for value in object_bbox]
    x0 = max(0, ox0 - max(0, int(padding)))
    y0 = max(0, oy0 - max(0, int(padding)))
    x1 = min(frame_width - 1, ox1 + max(0, int(padding)))
    y1 = min(frame_height - 1, oy1 + max(0, int(padding)))
    side = max(1, int(max_side))
    clipped = False
    if x1 - x0 + 1 > side:
        center = (ox0 + ox1) // 2
        x0 = max(0, min(frame_width - side, center - side // 2))
        x1 = min(frame_width - 1, x0 + side - 1)
        clipped = True
    if y1 - y0 + 1 > side:
        center = (oy0 + oy1) // 2
        y0 = max(0, min(frame_height - side, center - side // 2))
        y1 = min(frame_height - 1, y0 + side - 1)
        clipped = True
    clipped = clipped or x0 > ox0 or y0 > oy0 or x1 < ox1 or y1 < oy1
...
def _action_diffs(
    ...
    config: V8Config,
    *,
    object_rebindings: dict[str, str] | None = None,
) -> list[dict[str, Any]]:
    allowed = set(allowed_action_ids)
    records = [
        record
        for record in memory.action_memory_records
        if str(record.get("action_id") or "") in allowed and _record_matches_snapshot_level(record, snapshot)
    ]
    groups = _group_action_diff_records(
        records,
        focus_ids,
        allowed_action_ids,
        max(0, int(config.max_action_diffs_in_packet)),
        object_rebindings=object_rebindings or {},
    )
    object_aliases = aliases.get("object_real_to_alias", {})
    ...
    for group in groups:
        ...
        for change in record.get("object_deltas") or []:
            if not isinstance(change, dict):
                continue
            internal_id = str(change.get("object_id") or "")
            current_id = (object_rebindings or {}).get(internal_id, internal_id)
            if current_id not in focus_ids or current_id not in object_aliases:
                continue
            before = _factual_object_state(change, "before")
            after = _factual_object_state(change, "after")
            shared_attributes: dict[str, Any] = {}
            for key in ("palette_ids", "area", "shape_signature"):
                if key in before and key in after and before[key] == after[key]:
                    shared_attributes[key] = before.pop(key)
                    after.pop(key, None)
            object_changes.append(_drop_empty({
                "object_id": object_aliases[current_id],
                "lifecycle": change.get("lifecycle"),
                "before": before,
                "after": after,
                "shared_attributes": shared_attributes,
                "changed_cells_in_before_bbox": change.get("changed_cells_in_before_bbox"),
                "changed_cells_in_after_bbox": change.get("changed_cells_in_after_bbox"),
            }))
        all_changed_cells = [
            {
                "xy": [int(cell["xy"][0]), int(cell["xy"][1])],
                "before": str(cell.get("before")),
                "after": str(cell.get("after")),
            }
            for cell in record.get("changed_cells_xy") or []
            if isinstance(cell, dict) and isinstance(cell.get("xy"), (list, tuple)) and len(cell["xy"]) == 2
        ]
        changed_cells = all_changed_cells[: max(0, int(config.max_changed_cells_per_action_diff))]
        all_changed_runs = [
            {
                "y": int(run["y"]),
                "x_range_inclusive": [int(run["x_range_inclusive"][0]), int(run["x_range_inclusive"][1])],
                "before": str(run.get("before") or ""),
                "after": str(run.get("after") or ""),
            }
            for run in record.get("changed_cell_runs") or []
            if (
                isinstance(run, dict)
                and run.get("y") is not None
                and isinstance(run.get("x_range_inclusive"), (list, tuple))
                and len(run["x_range_inclusive"]) == 2
            )
        ]
        changed_runs = _sample_evenly(
            all_changed_runs,
            max(0, int(config.max_changed_row_runs_per_action_diff)),
        )
        changed_count = int(record.get("changed_cell_count") or 0)
        use_sparse_cells = changed_count <= 24 and (bool(all_changed_cells) or not all_changed_runs)
        represented_count = (
            len(changed_cells)
            if use_sparse_cells
            else sum(max(0, run["x_range_inclusive"][1] - run["x_range_inclusive"][0] + 1) for run in changed_runs)
        )
        pixel_diff = {
            "changed_cell_count": changed_count,
            "position_encoding": "sparse_cells_xy" if use_sparse_cells else "row_runs",
            "changed_cells_xy": changed_cells if use_sparse_cells else [],
            "changed_row_runs": [] if use_sparse_cells else changed_runs,
            "row_run_count": len(all_changed_runs) if not use_sparse_cells else 0,
            "position_data_complete": (
                represented_count == changed_count
                and (len(changed_cells) == len(all_changed_cells) if use_sparse_cells else len(changed_runs) == len(all_changed_runs))
            ),
        ...
        item = {
            ...
            "observation_summary": _action_diff_observation_summary(group, record),
            "coordinate_candidate_id": _alias_optional_id(record.get("coordinate_candidate_id"), aliases.get("candidate_real_to_alias", {})),
            ...
            "before": {
                "frame_hash": record.get("grid_hash_before"),
                "available_action_ids": record.get("available_actions_before") or [],
            },
            "after": {
                "frame_hash": record.get("grid_hash_after"),
                "available_action_ids": record.get("available_actions_after") or [],
            },
            ...
            "level_result": {
                ...
                "terminal": bool(record.get("terminal_delta")),
            },
        }
...
def _group_action_diff_records(
    ...
    limit: int,
    *,
    object_rebindings: dict[str, str],
) -> list[list[dict[str, Any]]]:
    if limit <= 0:
        return []
    grouped: dict[tuple[Any, ...], list[dict[str, Any]]] = {}
    for record in records:
        signature = _action_diff_record_signature(record, focus_ids, object_rebindings)
        grouped.setdefault(signature, []).append(record)
    groups = list(grouped.values())
    if len(groups) <= limit:
        return sorted(groups, key=_action_diff_group_order)

    priority: list[list[dict[str, Any]]] = []
    for action_id in allowed_action_ids:
        candidates = [group for group in groups if str(group[-1].get("action_id") or "") == str(action_id)]
        if candidates:
            priority.append(max(candidates, key=_action_diff_group_order))
    priority.extend(sorted(
        (group for group in groups if _action_diff_group_is_special(group)),
        key=_action_diff_group_order,
        reverse=True,
    ))
    priority.extend(sorted(groups, key=_action_diff_group_order, reverse=True))
    ...
    for group in priority:
        ...
        if len(selected) >= limit:
            break
    return sorted(selected, key=_action_diff_group_order)


def _local_visual_transition_evidence(record: dict[str, Any]) -> dict[str, Any]:
    raw = record.get("raw_visual_changes")
    if not isinstance(raw, dict) or not raw:
        return {}
    surface_changed = bool(
        record.get("planning_action_surface_added")
        or record.get("planning_action_surface_removed")
        or record.get("action_surface_added")
        or record.get("action_surface_removed")
    )
    repeated_interior = raw.get("repeated_isolated_interior_change") is True
    if not surface_changed and not repeated_interior:
        return {}

    patches = []
    for group in (raw.get("local_3x3_transition_groups") or [])[:6]:
        if not isinstance(group, dict):
            continue
        before_rows = [str(row) for row in (group.get("before_3x3_rows") or [])[:3]]
        after_rows = [str(row) for row in (group.get("after_3x3_rows") or [])[:3]]
        if not before_rows or not after_rows:
            continue
        patches.append(_drop_empty({
            "encoding": "categorical_hex_rows_0-F",
            "before_3x3_rows": before_rows,
            "after_3x3_rows": after_rows,
            "changed_offsets_xy": (group.get("changed_offsets_xy") or [])[:9],
            "occurrence_count": group.get("occurrence_count"),
            "interior_location_count": group.get("interior_location_count"),
            "edge_location_count": group.get("edge_location_count"),
            "locations_bbox_xyxy": group.get("locations_bbox_xyxy"),
            "sample_locations_xy": (group.get("locations_xy") or [])[:4],
        }))

    isolated_groups = []
    for group in (raw.get("isolated_center_cell_transition_groups") or [])[:6]:
        if not isinstance(group, dict):
            continue
        isolated_groups.append(_drop_empty({
            "before_value": group.get("before_value"),
            "after_value": group.get("after_value"),
            "occurrence_count": group.get("occurrence_count"),
            "interior_location_count": group.get("interior_location_count"),
            "edge_location_count": group.get("edge_location_count"),
            "locations_bbox_xyxy": group.get("locations_bbox_xyxy"),
    ...
    return _drop_empty({
        "scope": "SAME_OFFICIAL_ACTION_TRANSITION",
        "interpretation_status": "FACTUAL_SYNCHRONOUS_EVIDENCE_NOT_ACTION_SEMANTICS",
        "visual_and_action_surface_changed_together": bool(surface_changed and int(raw.get("changed_cell_count") or 0) > 0),
        "repeated_isolated_interior_change": repeated_interior,
        "isolated_center_cell_transition_groups": isolated_groups,
        "local_hex_transition_patches": patches,
        "reciprocal_isolated_interior_transition_pairs": (raw.get("reciprocal_isolated_interior_transition_pairs") or [])[:6],
    })


def _action_diff_record_signature(
    record: dict[str, Any],
    focus_ids: set[str],
    object_rebindings: dict[str, str],
) -> tuple[Any, ...]:
    ...
    for change in record.get("object_deltas") or []:
        if not isinstance(change, dict):
            continue
        historical_id = str(change.get("object_id") or "")
        object_id = object_rebindings.get(historical_id, historical_id)
        if object_id not in focus_ids:
            continue
        object_patterns.append((
            object_id,
            str(change.get("lifecycle") or ""),
            _freeze_signature_value(change.get("delta_centroid_rc")),
            bool(change.get("shape_changed")),
            bool(change.get("palette_changed")),
            _freeze_signature_value(change.get("before_colors")),
            _freeze_signature_value(change.get("after_colors")),
            _numeric_delta(change.get("area_delta"), change.get("before_area"), change.get("after_area")),
        ))
    ...
    return (
        ...
        str(record.get("coordinate_candidate_id") or ""),
        _freeze_signature_value(record.get("coordinate_xy")),
        str(record.get("coordinate_cell_before") or ""),
        str(record.get("coordinate_cell_after") or ""),
        _freeze_signature_value(record.get("available_actions_before") or ()),
        _freeze_signature_value(record.get("available_actions_after") or ()),
        tuple(sorted(object_patterns)),
        _freeze_signature_value(color_delta.get("transitions") or {}),
        int(record.get("levels_completed_delta") or 0),
        int(record.get("level_index_delta") or 0),
        bool(record.get("game_over_delta")),
        bool(record.get("terminal_delta")),
    )


def _action_diff_group_order(group: list[dict[str, Any]]) -> tuple[int, int]:
    steps = [int(item.get("step_index") or -1) for item in group]
    return (max(steps, default=-1), min(steps, default=-1))


def _action_diff_group_is_special(group: list[dict[str, Any]]) -> bool:
    for record in group:
        if record.get("coordinate_candidate_id") is not None or record.get("coordinate_xy") is not None:
            return True
        if tuple(record.get("available_actions_before") or ()) != tuple(record.get("available_actions_after") or ()):
            return True
        if any((record.get("levels_completed_delta"), record.get("level_index_delta"), record.get("game_over_delta"), record.get("terminal_delta"))):
            return True
        if any(
            isinstance(change, dict)
            and (
                str(change.get("lifecycle") or "persisted") != "persisted"
                or bool(change.get("palette_changed"))
            )
            for change in record.get("object_deltas") or []
        ):
            return True
...
def _action_diff_observation_summary(group: list[dict[str, Any]], sample: dict[str, Any]) -> dict[str, Any]:
    steps = [int(item.get("step_index") or -1) for item in group]
    changed_counts = [int(item.get("changed_cell_count") or 0) for item in group]
    return {
        "observation_count": len(group),
        "step_index_range": [min(steps, default=-1), max(steps, default=-1)],
        "sample_step_index": int(sample.get("step_index") or -1),
        "changed_cell_count_range": [min(changed_counts, default=0), max(changed_counts, default=0)],
    }
...
def _freeze_signature_value(value: Any) -> Any:
    if isinstance(value, dict):
        return tuple(sorted((str(key), _freeze_signature_value(item)) for key, item in value.items()))
...
def _numeric_delta(explicit: Any, before: Any, after: Any) -> float | int | None:
...
def _factual_object_state(change: dict[str, Any], prefix: str) -> dict[str, Any]:
    return _drop_empty({
        "bbox_xyxy": _bbox_xyxy_from_rc(change.get(f"{prefix}_bbox_rc")),
        "centroid_xy": _centroid_xy_from_rc(change.get(f"{prefix}_centroid_rc")),
        "palette_ids": [_hex_symbol(value) for value in change.get(f"{prefix}_colors") or []],
        "area": change.get(f"{prefix}_area"),
        "shape_signature": change.get(f"{prefix}_shape_signature"),
    })


def _confirmed_effect_memory(memory: "GameMemory", aliases: dict[str, dict[str, str]], config: V8Config) -> list[dict[str, Any]]:
    records = sorted(
        memory.action_effects.values(),
        key=lambda item: (int(item.level_index), int(item.last_step), str(item.action_id)),
    )[-max(0, int(config.max_memory_notes_in_packet)) :]
    return [
        {
            "action_id": str(record.action_id),
            "observed_outcome": str(record.outcome),
            "observation_count": int(record.evidence_count),
            "confidence": round(float(record.confidence), 4),
            "last_observed_level": int(record.level_index),
            "last_observed_step": int(record.last_step),
        }
        for record in records
        if float(record.confidence) >= 0.45
    ]


def _completed_level_memory(memory: "GameMemory", snapshot: ARGALiteSnapshot, config: V8Config) -> list[dict[str, Any]]:
    by_level: dict[int, list[dict[str, Any]]] = {}
    for record in memory.action_memory_records:
        level_index = int(record.get("level_index_before", record.get("level_index", -1)) or 0)
        if level_index >= int(snapshot.level_index):
            continue
        by_level.setdefault(level_index, []).append(record)

    completed: list[dict[str, Any]] = []
    for level_index, records in sorted(by_level.items()):
        terminal = next((
            record for record in reversed(records)
            if bool(record.get("levels_completed_delta") or record.get("level_index_delta") or record.get("terminal_delta"))
        ), None)
        if terminal is None:
            continue
        hypothesis_id = str(terminal.get("hypothesis_id") or "")
        solution_records = [
            record for record in records
            if hypothesis_id and str(record.get("hypothesis_id") or "") == hypothesis_id
        ]
        if not solution_records:
            solution_records = [record for record in records if not is_action_research_source(record.get("source"))]
        evaluation = next((
            item for item in reversed(memory.trajectory_evaluations)
            if int(item.level_index) == level_index
            and (not hypothesis_id or str(item.hypothesis_id) == hypothesis_id)
            and str(item.reason_code) == "trajectory_completed_level"
        ), None)
        completed.append(_drop_empty({
            "level_index": level_index,
            "outcome": "LEVEL_COMPLETED",
            "historical_object_ids_are_level_local": True,
            "hypothesis_id": hypothesis_id or None,
            "solution_claim": terminal.get("hypothesis_claim"),
            "verified_action_runs": _compact_record_action_runs(solution_records, config.max_qwen_trajectory_steps),
            "verified_result": ({
                ...
                "reason": "official_level_transition_observed",
            }),
        }))
    return completed[-4:]


def _compact_record_action_runs(records: list[dict[str, Any]], max_steps: int) -> list[dict[str, Any]]:
    runs: list[dict[str, Any]] = []
    emitted = 0
    for record in records:
        if emitted >= max(1, int(max_steps)):
            break
        action_id = str(record.get("action_id") or "")
        if not action_id:
            continue
        candidate_id = record.get("coordinate_candidate_id")
        if runs and runs[-1]["action_id"] == action_id and runs[-1].get("coordinate_candidate_id") == candidate_id:
            runs[-1]["repeat"] += 1
        else:
...
def _attempt_memory(
    ...
    attempts = []
    for record in memory.level_attempt_records:
        if int(record.get("level_index", -1)) != int(snapshot.level_index):
            continue
        total = record.get("attempt_total_execution") if isinstance(record.get("attempt_total_execution"), dict) else {}
        goal = record.get("goal_execution") if isinstance(record.get("goal_execution"), dict) else {}
        research = record.get("research_phase") if isinstance(record.get("research_phase"), dict) else {}
        verifier_feedback = record.get("verifier_feedback") if isinstance(record.get("verifier_feedback"), dict) else {}
        rejected_proposals = _factual_rejected_proposals(verifier_feedback.get("rejections") or [])
        hypotheses = []
        for hypothesis in record.get("executed_hypotheses") or []:
            if not isinstance(hypothesis, dict):
                continue
            hypotheses.append(_drop_empty({
                "hypothesis_id": hypothesis.get("hypothesis_id"),
                "claim": hypothesis.get("semantic_hypothesis"),
                "action_runs": _factual_action_runs(hypothesis.get("action_runs") or []),
                "executed_step_count": hypothesis.get("executed_step_count"),
                "outcome": hypothesis.get("outcome"),
                "last_reason": hypothesis.get("last_transition_reason"),
            }))
        attempts.append(_drop_empty({
            "attempt_index": record.get("attempt_index"),
            "outcome": record.get("outcome"),
            "reset_trigger": record.get("reset_trigger"),
            "qwen_calls": record.get("qwen_calls"),
            "research_action_runs": _factual_action_runs(research.get("action_runs") or []),
            "executed_action_runs": _factual_action_runs(record.get("action_runs") or []),
            "observed_result": total.get("observed_result") or {},
            "goal_execution_result": goal.get("observed_result") or {},
            "exact_replay_forbidden": bool(goal.get("exact_replay_forbidden")) or any(
                bool(item.get("actions")) for item in rejected_proposals
            ),
            "executed_hypotheses": hypotheses,
            "rejected_proposals": rejected_proposals,
        }))
    feedback = bank.attempt_feedback(config.max_memory_notes_in_packet)
    payload = {
        "current_attempt_index": memory.current_attempt_index(snapshot.level_index),
        "previous_failed_attempts": attempts[-4:],
        "current_attempt_feedback": feedback,
    }
    return _alias_recent_evidence(payload, aliases)


def _factual_action_runs(raw_runs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    out = []
    for run in raw_runs:
        if not isinstance(run, dict) or not run.get("action_id"):
            continue
        out.append(_drop_empty({
            "action_id": run.get("action_id"),
            "repeat": run.get("count", run.get("repeat", 1)),
            "coordinate_candidate_id": run.get("coordinate_candidate_id"),
            "coordinate_xy": run.get("coordinate_xy"),
        }))
    return out


def _factual_rejected_proposals(raw_rejections: list[dict[str, Any]]) -> list[dict[str, Any]]:
    out = []
    for rejection in raw_rejections:
        if not isinstance(rejection, dict):
            continue
        actions = [str(value) for value in rejection.get("actions") or [] if value]
        item = _drop_empty({
            "reason": rejection.get("reason"),
            "hypothesis_id": rejection.get("hypothesis_id"),
            "action_id": rejection.get("action_id"),
            "coordinate_candidate_id": rejection.get("coordinate_candidate_id"),
            "trajectory_step_index_1_based": rejection.get("trajectory_step_index_1_based"),
            "available_action_ids_before_step": rejection.get("available_action_ids_before_step") or [],
            "observed_enabling_action_ids": rejection.get("observed_enabling_action_ids") or [],
            "detail": rejection.get("detail"),
            "previous_attempt_index": rejection.get("previous_attempt_index"),
            "previous_hypothesis_id": rejection.get("previous_hypothesis_id"),
            "previous_outcome": rejection.get("previous_outcome"),
            "previous_rejection_reason": rejection.get("previous_rejection_reason"),
            "claim": rejection.get("claim"),
            "actions": actions,
            "exact_replay_forbidden": bool(actions),
        })
        out.append(item)
    return out


def _alias_optional_id(value: Any, mapping: dict[str, str]) -> Any:
    if value is None:
        return None
...
def _filter_semantic_feedback_references(
    ...
    feedback["bindings"] = [
        item
        for item in feedback.get("bindings") or []
        if isinstance(item, dict)
    ...
    feedback["invariants"] = [
        item
        for item in feedback.get("invariants") or []
        if isinstance(item, dict)
        and set(map(str, item.get("subject_object_ids") or [])).issubset(object_ids)
    ]
    for item in feedback.get("trajectory_evaluations") or []:
        if not isinstance(item, dict):
            continue
        item["source_object_ids"] = [
            str(value) for value in item.get("source_object_ids") or [] if str(value) in object_ids
        ]
        item["reference_object_ids"] = [
            str(value) for value in item.get("reference_object_ids") or [] if str(value) in object_ids
        ]
        item["relation_ids"] = [
            str(value) for value in item.get("relation_ids") or [] if str(value) in relation_ids
        ]
...
def _state(snapshot: ARGALiteSnapshot, memory: "GameMemory", role: QwenRole) -> dict[str, Any]:
    entry_source = memory.current_attempt_entry_source(snapshot.level_index)
    prior_research_is_history_only = (
        role is QwenRole.PRIMARY
        and entry_source in {"research_entry_reset", "failed_attempt_reset", "game_over_level_reset"}
    )
    return {
        ...
        "trajectory_start": {
            "current_frame_png_is_execution_start": True,
            "object_layer_is_execution_start": True,
            "current_available_action_ids_are_execution_start": True,
            "entry_source": entry_source,
            "prior_research_actions_are_history_only": prior_research_is_history_only,
        },
    }
    ...
    if isinstance(value, dict):
        for key, child in list(value.items()):
            if key in {"object_id", "source_object_id", "target_object_id", "linked_object_id"} and child is not None:
                ...
                "source_object_ids", "reference_object_ids", "subject_object_ids", "inferred_object_ids",
    ...
    *,
    ...
    for idx, obj in enumerate(selected[: config.max_objects_in_packet]):
        ...
        item = {
            ...
            "symmetry_hints": list(getattr(obj, "symmetry_hints", ()) or ()),
        }
