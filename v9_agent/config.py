...
class V8Config:
    ...
    qwen_multimodal_enabled: bool = False
    ...
    max_changed_cells_per_action_diff: int = 256
    max_changed_row_runs_per_action_diff: int = 6
    ...
    max_semantic_feedback_bindings: int = 8
    max_semantic_feedback_trajectories: int = 8
    max_semantic_feedback_invariants: int = 12
    semantic_invariant_confirmation_count: int = 2
    ...
    # Competition orchestration. GAME_OVER is reset-capable; the game wall-clock
    # deadline is the only terminal limit. Zero keeps legacy telemetry fields
    # disabled rather than imposing an action or attempt cap.
    max_actions_per_game: int = 0
    max_actions_per_level: int = 0
    max_level_attempts: int = 0
    game_wall_clock_limit_seconds: int = 8000
...
def config_from_mapping(mapping: dict[str, Any] | None = None) -> V8Config:
    ...
    env_aliases = {
        ...
        "qwen_multimodal_enabled": ("ARC_QWEN_MULTIMODAL_ENABLED",),
        "include_component_graph_in_qwen_packet": ("ARC_V8_INCLUDE_COMPONENT_GRAPH",),
        "frame_png_cell_scale": ("ARC_V8_FRAME_PNG_CELL_SCALE",),
        "max_hex_patches_in_packet": ("ARC_V8_MAX_HEX_PATCHES",),
        "max_hex_patch_side": ("ARC_V8_MAX_HEX_PATCH_SIDE",),
        "max_action_diffs_in_packet": ("ARC_V8_MAX_ACTION_DIFFS",),
        "max_changed_cells_per_action_diff": ("ARC_V8_MAX_CHANGED_CELLS_PER_DIFF",),
        "max_changed_row_runs_per_action_diff": ("ARC_V9_MAX_CHANGED_ROW_RUNS_PER_DIFF",),
    ...
    if cfg.qwen_backend == "vllm" and cfg.qwen_enable_thinking:
        cfg = replace(cfg, qwen_reasoning_mode="on", qwen_reasoning_budget_tokens=max(0, cfg.qwen_reasoning_budget_tokens))
    else:
