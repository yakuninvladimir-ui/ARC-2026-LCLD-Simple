"""Build the Qwen3.6-27B FP8/vLLM ARC-AGI-3 Kaggle notebook.
...
NOTEBOOK_PATH = NOTEBOOK_ROOT / "arc-prize-2026-lcld-qwen.ipynb"
...
def _preflight_script_source() -> str:
    source = dedent(
        ...
        for name in ('kaggle_agent', 'submission', 'v8_agent', 'v8_agent.session', 'v8_agent.llm'):
        ...
        if full_import_sweep:
            for path in sorted((code_dir / 'v8_agent').glob('*.py')):
                if path.name == '__init__.py':
                    continue
                name = 'v8_agent.' + path.stem
                mod = importlib.import_module(name)
        ...
        assert delegate.config.get('qwen_reasoning_mode') == 'off', repr(delegate.config)
        assert int(delegate.config.get('qwen_reasoning_budget_tokens', -1)) == 0, repr(delegate.config)
        assert delegate.config.get('qwen_vllm_model') == 'vrfai/Qwen3.6-27B-FP8', repr(delegate.config)
        assert int(delegate.config.get('qwen_context_tokens', 0)) == 98304, repr(delegate.config)
        assert int(delegate.config.get('qwen_max_input_tokens', 0)) == 65536, repr(delegate.config)
        assert int(delegate.config.get('qwen_max_output_tokens', 0)) == 12288, repr(delegate.config)
...
def _adapt_working_phase_b_source(source: str) -> str:
    ...
    run_direct_game = dedent(
        ...
        def _run_direct_game(env, game_id):
            ...
            _trace(
                ...
                reset_policy='unconditional_initial_reset_then_tufa_single_reset_after_game_over',
            ...
            try:
                while accepted_actions < max_actions:
                    ...
                    is_initial_reset = bool(initial_reset_pending)
                    if is_initial_reset:
                ...
                return {
                    ...
                    'initial_reset_count': int(initial_reset_count),
    ...
    write_results = dedent(
        ...
        def _write_results(status, results, game_count):
            payload = {
                ...
                'execution_path': 'tufa_style_GAME_OVER_RESET_then_ARC_AGI_Agent_act_to_env_step',
                'unconditional_initial_reset': True,
                'one_attempt_game_over_terminal': False,
                'game_over_reset_policy': 'single_reset_before_next_qwen_call',
                'max_level_attempts': int(os.environ.get('LCLD_MAX_LEVEL_ATTEMPTS', '4')),
                'max_actions_per_game': int(os.environ.get('LCLD_MAX_ACTIONS_PER_GAME', '200')),
                'max_actions_per_level': int(os.environ.get('LCLD_MAX_ACTIONS_PER_LEVEL', '0')),
                'only_reset_levels': os.getenv('ONLY_RESET_LEVELS', '').lower() == 'true',
                ...
                'total_initial_resets': sum(int(item.get('initial_reset_count', 0) or 0) for item in results),
    ...
    source = _replace_function_block(source, "    def _run_direct_game(env, game_id):", "    def _write_results(status, results, game_count):", run_direct_game)
    ...
    source = _replace_exact_once(
        ...
        label='Phase-B default metrics reset counter',
    )
    source = _replace_exact_once(
        source,
        "                        'rejected_action_count': 0,\n                        'levels_completed': 0,",
        "                        'rejected_action_count': 0,\n                        'initial_reset_count': 0,\n                        'game_over_reset_count': 0,\n                        'levels_completed': 0,",
        label='Phase-B skipped metrics reset counter',
    )
    source = _replace_exact_once(
        source,
        "                'total_rejected_actions', 'levels_completed_observed',",
        "                'total_rejected_actions', 'total_initial_resets', 'total_game_over_resets', 'levels_completed_observed',",
        label='Phase-B final summary reset counter',
    )
    return source
...
def _adapt_working_common_source(source: str, preflight_script: str) -> str:
    payload_check = dedent(
        ...
        def _assert_payload_structure():
            ...
            if missing_payload:
                raise FileNotFoundError('Embedded V8.3 Code payload missing files: ' + repr(missing_payload))
...
def _current_agent_unpack_source(payload: str) -> str:
    return dedent(
        ...
        print('LCLD V8.3 source ready:', os.environ['ARC_AGENT_CODE_DIR'], flush=True)
        """
    )
...
def build() -> dict:
    ...
    common_source = _working_wrapper_source("working_common.py")
    return {
        "metadata": {
            "kaggle": {
        ...
        "cells": [
            ...
            code_cell(_adapt_working_phase_b_source(_working_wrapper_source("working_phase_b.py"))),
